...
<div class="card mb-3">
	...
	<div class="card-body">
		<sf:form class="form-horizontal" modelAttribute="product"
			action="${contextRoot}/laboratory/product" method="POST"
				...
				<table>
					...
					<c:if test="${supplier != null}">
						<tr>
							...
							<td>${supplier.nameOfEstablishment }<sf:input type="hidden"
									path="supplierId" value="${supplier.id}" />
							</td>
					...
					<c:if test="${supplier == null}">
						<tr>
							...
							<td><sf:select path="supplierId" items="${suppliers}"
					...
					<tr>
						...
						<td>
							<c:if test="${edit}">
								<sf:input path="code" class="form-control"
									placeholder="Product Code" required="required" readonly="true" />
							</c:if> <c:if test="${edit eq false}">
								<sf:input path="code" class="form-control"
					...
					<tr>
						...
						<td><sf:input path="name" class="form-control"
								placeholder="Product Name" required="required" /></td>
					</tr>
					...
					<tr>
						...
						<td><c:if test="${supplier != null}">
								<a
									href="${contextRoot}/laboratory/supplier/${supplier.id}/products"
									class="btn btn-secondary">Cancel</a>
							</c:if> <c:if test="${supplier == null}">
								<a href="${contextRoot}/laboratory/products"
									class="btn btn-secondary">Cancel</a>
							</c:if> <input type="submit" value="Save" class="btn btn-primary" /></td>
					</tr>
				</table>
			</div>
		</sf:form>
